...
public class Task4_Exam {
    ...
    public static TruckType getTypeByWeight(int weight) {
        return Arrays.stream(TruckType.values()) //тут я гпт спросил ибо я не знал как сделать стрим на основе значений энама
                .filter(type -> type.canHandleWeight(weight))
                .findFirst()
                .orElseThrow(WeightTooHighException::new); //а тут я нихуя не понял запись и тоже гуглил
    }
